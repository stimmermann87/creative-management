 ...
 CREATE OR REPLACE VIEW events AS
         WITH flattened AS (
             ...
             FROM (
                 ...
                 FROM read_json_auto(
                     'CreativesIntegration.Api/tracking-events.json'
                 )
             )
         )
         SELECT * REPLACE (
             TRY_CAST(clientTimestampUtc AS TIMESTAMPTZ) AS clientTimestampUtc,
             TRY_CAST(serverReceivedAtUtc AS TIMESTAMPTZ) AS serverReceivedAtUtc,
         )
         FROM flattened;
```
